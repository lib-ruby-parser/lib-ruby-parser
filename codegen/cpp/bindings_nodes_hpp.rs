...
fn contents() -> String {
    ...
    format!(
        "#ifndef LIB_RUBY_PARSER_EXTERNAL_CPP_BINDINGS_NODES_HPP
#define LIB_RUBY_PARSER_EXTERNAL_CPP_BINDINGS_NODES_HPP
...
#endif // LIB_RUBY_PARSER_EXTERNAL_CPP_BINDINGS_NODES_HPP",
        generator = file!(),
        // fns
...
pub(crate) fn codegen() {
    std::fs::write("external/cpp/bindings_nodes.hpp", contents()).unwrap();
}

fn constructor(node: &lib_ruby_parser_nodes::Node) -> String {
    format!("{sig};", sig = c_helpers::nodes::constructor::sig(node))
}
...
fn variant_getter(node: &lib_ruby_parser_nodes::Node) -> String {
    format!("{sig};", sig = c_helpers::nodes::variant_getter::sig(node))
}
fn field_getters(node: &lib_ruby_parser_nodes::Node) -> Vec<String> {
    node.fields
        .map(&|field| format!("{sig};", sig = c_helpers::nodes::getter::sig(node, field)))
}
fn field_setters(node: &lib_ruby_parser_nodes::Node) -> Vec<String> {
    node.fields
        .map(&|field| format!("{sig};", sig = c_helpers::nodes::setter::sig(node, field)))
}
fn internal_struct(node: &lib_ruby_parser_nodes::Node) -> String {
    let fields = node.fields.map(&|field| {
        format!(
            "{} {};",
            c_helpers::nodes::fields::blob_type(field),
...
fn into_variant_fn(node: &lib_ruby_parser_nodes::Node) -> String {
    format!("{sig};", sig = c_helpers::nodes::into_variant::sig(node))
}
fn into_internal_fn(node: &lib_ruby_parser_nodes::Node) -> String {
    format!("{sig};", sig = c_helpers::nodes::into_internal::sig(node))
}
fn variant_drop_fn(node: &lib_ruby_parser_nodes::Node) -> String {
    format!("{sig};", sig = c_helpers::nodes::drop_variant::sig(node))
}

...
pub(crate) fn codegen(node: &lib_ruby_parser_nodes::Node) {
    let path = format!("src/nodes/types/{}/tests.rs", node.filename);
    ...
    let fields = node
        ...
        .map(&|field| {
            ...
            let value = match field.field_type {
                ...
                NodeFieldType::Chars => &d_maybe_string_ptr,
                NodeFieldType::StringValue => &d_bytes,
                NodeFieldType::U8 => &d_u8,
                NodeFieldType::Usize => &d_usize,
                NodeFieldType::RawString => &d_string_ptr,
                NodeFieldType::RegexOptions => &d_maybe_node,
            };
            format!("{}: {}", key, value)
        })
        .join(", ");
...
fn make_field_fn(field: &lib_ruby_parser_nodes::NodeField) -> String {
    ...
    match field.field_type {
        ...
        NodeFieldType::Chars => "make_maybe_string_ptr",
        NodeFieldType::StringValue => "make_bytes",
        NodeFieldType::U8 => "make_u8",
        NodeFieldType::Usize => "make_usize",
        NodeFieldType::RawString => "make_string_ptr",
        NodeFieldType::RegexOptions => "make_maybe_node_ptr",
    }
    .to_string()
}

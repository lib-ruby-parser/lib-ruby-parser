...
impl LexChar {
    ...
    pub fn to_option(&self) -> Option<char> {
        match self {
            LexChar::Multibyte(c)
            | LexChar::AsciiByte(c) => Some(*c),
            _ => None
        }
    }

    pub fn is_ascii(&self) -> bool {
        if let Some(c) = self.to_option() { c.is_ascii() } else { false }
    }

    pub fn is_upper(&self) -> bool {
        if let Some(c) = self.to_option() { c.is_ascii_uppercase() } else { false }
    }

    pub fn is_lower(&self) -> bool {
        if let Some(c) = self.to_option() { c.is_ascii_lowercase() } else { false }
    }

    pub fn is_alpha(&self) -> bool {
        if let Some(c) = self.to_option() { c.is_ascii_alphabetic() } else { false }
    }

    pub fn is_digit(&self) -> bool {
        if let Some(c) = self.to_option() { c.is_ascii_digit() } else { false }
    }

    pub fn is_alnum(&self) -> bool {
        if let Some(c) = self.to_option() { c.is_ascii_alphanumeric() } else { false }
    }

    pub fn is_hexdigit(&self) -> bool {
        if let Some(c) = self.to_option() { c.is_ascii_hexdigit() } else { false }
    }

    pub fn is_blank(&self) -> bool {
        if let Some(c) = self.to_option() { c == SPACE || c == TAB } else { false }
    }

    pub fn is_space(&self) -> bool {
        if let Some(c) = self.to_option() { c.is_ascii_whitespace() || c == VTAB } else { false }
    }

    pub fn is_global_name_punct(&self) -> bool {
        if let Some(c) = self.to_option() { PUNCT.contains(&c) } else { false }
    }

    pub fn is_control(&self) -> bool {
        if let Some(c) = self.to_option() { c.is_control() } else { false }
    }
    ...
    pub fn map_as_u8<F: FnOnce(u8) -> u8>(&self, f: F) -> LexChar {
        match &self {
            ...
            LexChar::NonUtf8Byte(c) => {
                LexChar::new(f(*c))
            },
            LexChar::AsciiByte(c) => {
                LexChar::new(f(*c as u8))
            },
